...
export function registerReconciliationRoutes(app: FastifyInstance, c: Container, queue?: Queue) {
  ...
  app.post("/reconciliations", async (req, reply) => {
    ...
    if (!win) {
      return reply
        .code(400)
        .send(failure("windowStart/windowEnd required (start < end)"));
    }

    if (queue) {
      const created = await c.repo.createRun(win.start, win.end);
      if (!created.ok) return reply.code(500).send(failure(created.error.message));
      await queue.enqueue({
        runId: created.value.id,
        windowStart: win.start.toISOString(),
        windowEnd: win.end.toISOString(),
      });
      return reply.code(202).send(data({ status: "queued", runId: created.value.id }));
    }

    const result = await c.reconcileRun.execute(win.start, win.end);
    if (!result.ok) return reply.code(500).send(failure(result.error.message));
    return reply.code(201).send(data({ status: "done", runId: result.value.runId }));
  });
